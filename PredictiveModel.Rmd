...
########################
set.seed(9876)
enmod <- glmnet(trainx, trainy, alpha = 0.25)
cvout2 <- cv.glmnet(trainx, trainy, alpha = 0.25)
lambdaen <- cvout2$lambda.min
lambdaen
enpred <- predict(enmod, s = lambdaen, newx = x[test,])
mean((enpred-testy)^2)
outer <- glmnet(x, y, alpha = 0.25)
enmodel <- predict(outer, type = "coefficients", s = lambdaen)[1:19,]
enmodel[enmodel!=0]
